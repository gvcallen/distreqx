...
class Combined(
    ...
    ```python
    import jax
    from distreqx.distributions import Combined, Joint, Normal, Uniform

    a = Joint({"x": Normal(0.0, 1.0), "y": None})
    b = Joint({"x": None, "y": Uniform(3.8, 4.2)})
    dist = Combined((a, b))

    sample = dist.sample(jax.random.key(0))  # {"x": ..., "y": ...}
